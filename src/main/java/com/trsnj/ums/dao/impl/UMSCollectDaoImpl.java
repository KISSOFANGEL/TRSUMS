 ...
 * @since V1.00
 */
public class UMSCollectDaoImpl extends BaseDao<UMSCollect> implements IUMSCollectDao
{
     ...
     */
    @Override
    public Class<UMSCollect> getEntityClass()
    {
        return UMSCollect.class;
    }
    /**
     * 获取当前用户的搜藏信息
     * @param userId
    ...
    public  List<UMSCollect> getCollectByUserId(long userId){
        String sql="select * from UMSCollect where userId ="+userId;
        Session session=getCurrentSession();
        Query query=null;
        query=session.createSQLQuery(sql).addEntity(getEntityClass());
        List<UMSCollect> collects=query.list();
        return collects;
    }
    /**
     * 分页获取当前用户的搜藏
    ...
    public long getCollectCountByUserId(long userId){
        String sql="select count(*) from UMSCollect where userId ="+userId;
        Session session=getCurrentSession();
        Query query=null;
    ...
    public String delCollectByIds(long userId,String ids){
        String sql = "delete from umscollect where userid ="+userId+" and UMSCollectid in("+ids+")";
    ...
    public List<String> hotRecommendDoc(long userid, int first, int max){
        String sql="select docid from umscollect where docid not in (select docid from umscollect where userid ="+userid+")  group by(docid) order by count(docid) desc";
        Session session=getCurrentSession();
        Query query=null;
        query=session.createSQLQuery(sql)
    ...
    public UMSCollect getCollectByUserIdAndDocid(String docid, String userId){
        String sql="select * from UMSCollect where userId ="+userId+" and docid ="+docid;
        Session session=getCurrentSession();
        Query query=null;
        query=session.createSQLQuery(sql).addEntity(getEntityClass());
        List<UMSCollect> collects=query.list();
