...
var perpage=10;  //每页数
	var html="<tr><th><input type='checkbox' id='checkall'  onclick='changecheckbox()'/>全选</th><th>操作对象</th><th>操作事项</th><th>操作时间</th></tr>";
	$.ajax({
		type:"POST",
		url:"<%=root%>/behaviorlogs/getMyDynamic",
		dataType:"json",
		async:false,
		...
		success:function(result){
			...
			for(i in result){
				...
				html = html +"<tr><td><input type='checkbox' name='collids' value='"+result[i].dynamicid+"'/>"+k+"</td><td>"+result[i].objname+"</td><td>"+result[i].action+"</td><td>"+result[i].operatetime+"</td></tr>";
...
function delselectedmes(){
     ...
     $.ajax({
 		type:"POST",
 		url:"<%=root%>/behaviorlogs/delMyDynamic",
 		dataType:"html",
 		async:false,
 		data:{ids:ids},
 		success:function(result){
  ...
  <div class="am-cf am-padding">
      <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">我的动态</strong> / <small>Dynamic</small></div>
    </div>
