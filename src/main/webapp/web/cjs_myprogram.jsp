...
var perpage=10;  //每页数
	var html="<tr><th><input type='checkbox' id='checkall'  onclick='changecheckbox()'/>全选</th><th>文章标题</th><th>所属栏目</th><th>收藏时间</th></tr>";
	$.ajax({
		type:"POST",
		url:"<%=root%>/program/getProgramByUserId",
		dataType:"json",
		async:false,
		...
		success:function(result){
			...
			for(i in result){
				...
				html = html +"<tr><td><input type='checkbox' name='collids' value='"+result[i].collectid+"'/>"+k+"</td><td><label  style='cursor:pointer;font-weight:normal;line-height:normal' title='"+result[i].doctitle+"'  ><a href='"+result[i].puburl+"' target='_blank'>"+result[i].doctitle.substring(0,10)+"</a></label></td><td>"+result[i].chnlname+"</td><td>"+result[i].coltime+"</td></tr>";
...
function delselectedmes(){
     ...
     $.ajax({
 		type:"POST",
 		url:"<%=root%>/collect/delProgramByIds",
 		dataType:"html",
 		async:false,
 		data:{ids:ids},
 		success:function(result){
  ...
  <div class="am-cf am-padding">
      <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">关注的项目</strong> / <small>Collection</small></div>
    </div>
...
<span id="totalcounts"></span>
  <div class="am-fr"  id="pagination" >
   ...
   <div class="panel-heading">
      <h3 class="panel-title">我的收藏<button type='button' class='btn btn-danger delbtn' onclick="infoalert()" style="float:right"
 >删除选中收藏</button></h3>
   </div>
