...
<div class="div_grid">
	<!-- 当前角色下面的数据列表加分页-->
	<table id="groupuserTable" title="用户列表" cellspacing="0" cellpadding="0" width="100%" iconCls="icon-edit"></table>
...
<div id="userAdd"   icon="icon-save" class="easyui-dialog alldialog">
	<!-- 非当前角色下的用户列表 -->
	<table id="userTableNotInGroup" title="组织用户列表" cellspacing="0" cellpadding="0" width="100%" iconCls="icon-edit"></table>
	<div class="div_ul">
	   <ul>
	     <li align="center">
   ...
   /**--------加载表格数据 ------*/
	function groupuserTable(){
		//加载表格
		$('#groupuserTable').datagrid({
			url: 'getUsersByGroupId?id='+<%=groupId%>,   //这里的命名空间默认是action配置跳转到里面时的空间，所以不加命名空间
	...
	function userTableNotInGroup(){
		//加载表格
		$('#userTableNotInGroup').datagrid({
			url: 'getUsersNotInNowGroupId?id='+<%=groupId%>,  //这里的命名空间默认是action配置跳转到里面时的空间，所以不加命名空间
			//toolbar:'#toolbar',//这是div的id属性值
			checkOnSelect:true,
			pagination:true,//是否分页
			fitColumns:true,//列自适应表格宽度
	...
	//执行用户添加操作    
	function addUser(){
			if($('#userTableNotInGroup').datagrid('getSelected')){
				var ids = [];
				var ids_yz =[];
				var dpnames = [];
				var selectedRow = $('#userTableNotInGroup').datagrid('getSelections');
				var count=0;
				...
				var dpname =dpnames.join(","); 
					$.messager.confirm('提示', '是否将选中的用户添加到当前组织中? '+"\n"+dpname,function(r){
						//首先如果用户选择了数据，则获取选择的数据集合
						if(r){
							updateGroup(dpid,ids_yz);
						}
					});
			}
	}
	function updateGroup(dpid,ids_yz){
		var url ='updateUserToGroupId';
		var options = {	url:url,
						data:{"ids":dpid,
							  "ids_yz":ids_yz,
							  "id":<%=groupId%>
						...
						callBackFun:function(data){  //凡是ajax请求的action返回的都是BaseEntity的json对象
							if(data.isSuccessOrfail=="SUCCESS"){
								// 重新加载两个表格数据
								reloadTable("userTableNotInGroup");
								reloadTable("groupuserTable");
							}
							showMessage(data);
						}
	...
	**/
	function delUser(id){	
		$.messager.confirm('提示', '你确定要删除当前组织下的该用户吗?',function(r){
			if(r){
				var url ='delUserInNowGroupId';
				var options = {
						url:url,
						data:{"id":id,"gid":<%=groupId%>},
